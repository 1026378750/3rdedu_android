...
public abstract class AbsAPICallback<T> extends Subscriber<T> {
    ...
    public void onError(Throwable e) {
        ...
        if (e instanceof HttpException) {//HTTP错误
            ...
            LogUtils.d(UiUtils.getString( R.string.socketTimeOutException));
    ...
    private void switchError(ResultException resultException){
        switch (resultException.getErrCode()){
            ...
            case IntegerUtil.ABS_TOKEN_EXPIRE://您的账号已过期，为确保信息安全，请重新登录！
                ...
                ConfirmDialog dialog = ConfirmDialog.newInstance("", UiUtils.getString(R.string.tokenVerifyExpire) , "", "立即登录");
                dialog.setMargin(60)
            ...
            case IntegerUtil.ABS_TOKEN_POST_NULL://请求token不能为空
                ...
                LoginOpentionUtil.getInstance().LoginRequest(mContext);
                break;
            case IntegerUtil.ABS_USER_VERIFICATION://验证码超时，请重新获取
                ToastUtil.showShort(UiUtils.getString(R.string.userSecurity));
                break;
            case IntegerUtil.ABS_USER_GETMOBILE://获取手机验证码失败
                ToastUtil.showShort(UiUtils.getString(R.string.userMoblie));
                break;
            case IntegerUtil.ABS_USER_PARAMETER://获取手机验证码失败
                ToastUtil.showShort(UiUtils.getString(R.string.userParameterFail));
                break;
            case IntegerUtil. ABS_USER_VERIFIAGAIN://获取手机验证码失败
                ToastUtil.showShort(UiUtils.getString(R.string.userSecurityAgain));
                break;
            case IntegerUtil. PERMISSION_SYSEX_SETTING://系统异常
                //ToastUtil.showShort(UiUtils.getString(R.string.systemExpire));
                break;
            default:
                onResultError(resultException);
                break;
