 ...
 */
public class PickerToolsView extends View {
    ...
    public PickerToolsView(Context context, AttributeSet attrs) {
        super(context, attrs);
     ...
     * @param mMaxTextSize
     */
    public void setTextSizeRange(float mMinTextSize,float mMaxTextSize){
        if(mMinTextSize==0||mMaxTextSize==0||mMaxTextSize<mMinTextSize){
            return;
        }
    ...
    private void init() {
        ...
        mPaint.setColor(UiUtils.getColor(R.color.color_333333));
        //第二个paint
        nPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        nPaint.setStyle(Style.FILL);
        nPaint.setTextAlign(Align.CENTER);
        nPaint.setColor(UiUtils.getColor(R.color.text2));
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        // 根据index绘制view
        if (isInit&&mDataList!=null&&!mDataList.isEmpty()) {
            drawData(canvas);
        }
    }
